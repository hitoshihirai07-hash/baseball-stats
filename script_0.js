        ...
        function updateHomeCounts() {
            ...
            const battingCount = (window.BATTING_DATA?.length || summaryCounts.batting || 0);
            const pitchingCount = (window.PITCHING_DATA?.length || summaryCounts.pitching || 0);
            const orderCount = (window.ORDER_DATA?.length || summaryCounts.order || 0);
            const set = (id, value) => {
                const el = document.getElementById(id);
                if (el) el.textContent = value;
            };
            set('home-batting-count', battingCount + '件');
            set('home-pitching-count', pitchingCount + '件');
            set('home-order-count', orderCount + '試合');
        }
        ...
        function checkSim() {
            ...
            const batSide = document.getElementById('sim-bat-side')?.value || 'R';
            const pitHand = document.getElementById('sim-pit-hand')?.value || 'R';
            ...
            if (curBS && curPS) {
                ...
                if (note) note.textContent = `打者は対${pitHand === 'L' ? '左' : '右'}、投手は対${batSide === 'L' ? '左' : '右'}打者成績で補正します。`;
            } else {
                btn.disabled = true;
        ...
        function handLabel(hand, type = 'pit') {
            if (type === 'bat') return hand === 'L' ? '左打席' : '右打席';
            return hand === 'L' ? '左投手' : '右投手';
        }
        ...
        function getAdjustedBatterMetrics(bData, pitcherHand) {
            ...
            const splitAvg = splitRow ? (pitcherHand === 'L' ? splitRow.leftAvg : splitRow.rightAvg) : null;
            const splitAb = splitRow ? (pitcherHand === 'L' ? splitRow.leftAb : splitRow.rightAb) : null;
            const splitPa = splitRow ? (pitcherHand === 'L' ? splitRow.leftPaAb : splitRow.rightPaAb) : null;
            if (typeof splitAvg === 'number' && Number.isFinite(splitAvg)) {
                ...
                return {
                    ...
                    adjusted: true,
                    splitAvg,
                    label: `対${pitcherHand === 'L' ? '左' : '右'} ${fmtRate(splitAvg)}`
                };
            }
            return {
                avg: seasonAvg,
        ...
        function getAdjustedPitcherMetrics(pData, batterSide) {
            ...
            const splitAvg = splitRow ? (batterSide === 'L' ? splitRow.leftAvg : splitRow.rightAvg) : null;
            if (typeof splitAvg === 'number' && Number.isFinite(splitAvg)) {
                ...
                return {
                    ...
                    adjusted: true,
                    splitAvg,
                    label: `対${batterSide === 'L' ? '左' : '右'}打者 ${fmtRate(splitAvg)}`
                };
            }
            return {
    ...
    <div class="card">
      ...
      <div class="cd">${r.detail}</div>
            ...
            // Champ bars
            const champMap = {};
            TEAM_DATA.forEach(d => { if (d.nippon === '○') champMap[d.team] = (champMap[d.team] || 0) + 1; });
            const champs = Object.entries(champMap).sort((a, b) => b[1] - a[1]);
            ...
            document.getElementById('chbars').innerHTML = champs.map(([t, c]) => {
      ...
      <div class="bar-num" style="color:${col}">${c}回</div>
        ...
        async function show(id) {
            ...
            const subs = {
                home: '打順・成績・分析を横断して見られる野球データサイト',
            ...
            const summaryCounts = window.SITE_SUMMARY?.counts || {};
            const counts = {
                home: ((window.BATTING_DATA?.length || summaryCounts.batting || 0) + (window.PITCHING_DATA?.length || summaryCounts.pitching || 0)) + '件',
        ...
        function initOrderBuilder() {
            ...
            const handSel = document.getElementById('mo-vs-hand');
            if (handSel && !handSel.value) handSel.value = 'R';
        ...
        function getOrderPitcherHand() {
            return document.getElementById('mo-vs-hand')?.value || 'R';
        }
